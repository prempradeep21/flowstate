...
import {
  EMERGENT_CHAPTERS,
  EMERGENT_DISTRICTS,
  EMERGENT_SCOREBOARD_ROWS,
  EMERGENT_SCOREBOARD_STICKIES,
  EMERGENT_SEED_CARD,
  EMERGENT_SUBTITLE,
  EMERGENT_TIMELINE,
} from "@/lib/sampleCanvases/emergent/data";
import {
...
const ROOT_CARD_ID = "emg-card-root";
const THREAD_ID = "emg-thread-main";
...
export function buildEmergentCanvas(): CanvasSnapshot {
  ...
  const cards: Record<string, Card> = {
    [ROOT_CARD_ID]: {
      id: ROOT_CARD_ID,
      threadId: THREAD_ID,
      question: EMERGENT_SEED_CARD.question,
      answer: EMERGENT_SEED_CARD.answer,
      status: "done",
  ...
  const placeArtifact = (
    ...
    const stableId = `emg-art-${spec.key}`;
    ...
    const nodeId = `emg-node-${spec.key}`;
  ...
  let rowsRightEdge = 0;
  for (const row of EMERGENT_SCOREBOARD_ROWS) {
  ...
  {
    const sizes = EMERGENT_SCOREBOARD_STICKIES.map(specSize);
    const positions = columnPositions(
      sizes,
      { x: scoreboardStickyX, y: SCOREBOARD_ROWS_START_Y },
      STICKY_GAP_Y,
    );
    EMERGENT_SCOREBOARD_STICKIES.forEach((spec, i) => {
      const size = placeArtifact(spec, positions[i]!);
      scoreboardRects.push({ ...positions[i]!, ...size });
    });
  }
  ...
  const stickyBottom =
    SCOREBOARD_ROWS_START_Y +
    stackHeight(EMERGENT_SCOREBOARD_STICKIES.map(specSize), STICKY_GAP_Y);
  const timelineY = Math.max(rowY, stickyBottom + NODE_GAP_Y);
  const timelineSize = placeArtifact(EMERGENT_TIMELINE, { x: 0, y: timelineY });
  ...
  const bands = districtBands(EMERGENT_DISTRICTS, districtsStartY);

  EMERGENT_DISTRICTS.forEach((district, districtIndex) => {
  ...
  let chapterX = 0;
  EMERGENT_CHAPTERS.forEach((chapter, chapterIndex) => {
