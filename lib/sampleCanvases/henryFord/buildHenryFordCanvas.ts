...
import {
  HFORD_ERAS,
  HFORD_OVERVIEW_ROWS,
  HFORD_OVERVIEW_STICKIES,
  HFORD_SEED_CARD,
  HFORD_SUBTITLE,
  HFORD_TIMELINE,
  type SampleArtifactSpec,
} from "@/lib/sampleCanvases/henryFord/data";
import {
...
const ROOT_CARD_ID = "hford-card-root";
const THREAD_ID = "hford-thread-main";
...
export function buildHenryFordCanvas(): CanvasSnapshot {
  ...
  const cards: Record<string, Card> = {
    [ROOT_CARD_ID]: {
      id: ROOT_CARD_ID,
      threadId: THREAD_ID,
      question: HFORD_SEED_CARD.question,
      answer: HFORD_SEED_CARD.answer,
      status: "done",
  ...
  const placeArtifact = (
    ...
    const stableId = `hford-art-${spec.key}`;
    ...
    const nodeId = `hford-node-${spec.key}`;
  ...
  {
    const sizes = HFORD_OVERVIEW_STICKIES.map(specSize);
    const positions = columnPositions(
      sizes,
      { x: OVERVIEW_STICKY_X, y: OVERVIEW_ROWS_START_Y },
      STICKY_GAP_Y,
    );
    HFORD_OVERVIEW_STICKIES.forEach((spec, i) => {
  ...
  for (const row of HFORD_OVERVIEW_ROWS) {
