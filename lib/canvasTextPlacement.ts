...
/** World position for a text label spawned beside a selection rect. */
...
/** World position for a text label spawned beside artifact (or panel) text selection. */
export function computeArtifactSelectionTextLabelPosition(
  selectionRect: DOMRect,
  viewport: { x: number; y: number; scale: number },
): { x: number; y: number } | null {
  const container = document.querySelector<HTMLElement>("[data-canvas-container]");
  if (!container) return null;
  const containerRect = container.getBoundingClientRect();
  const clientX = selectionRect.right + LABEL_SPAWN_GAP_PX;
  const clientY = selectionRect.top + selectionRect.height / 2;
  const { worldX, worldY } = clientToWorld(
    clientX,
    clientY,
