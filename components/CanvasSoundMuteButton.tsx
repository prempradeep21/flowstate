...
export function CanvasSoundMuteButton() {
  ...
  if (!mounted) {
    return (
      <div
        className="pointer-events-none absolute bottom-5 left-5 z-[55] h-9 w-9 rounded-canvas"
        aria-hidden
      />
    );
  }
  ...
  return (
    <button
      ...
      aria-pressed={muted}
      className={`pointer-events-auto absolute bottom-5 left-5 z-[55] flex h-9 w-9 items-center justify-center rounded-canvas border border-canvas-border bg-canvas-card/95 text-canvas-muted shadow-card backdrop-blur-sm transition-colors hover:bg-canvas-bg hover:text-canvas-ink ${
