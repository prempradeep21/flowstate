...
function screenViewport(
  camera: DisneySceneState["camera"],
...
function buildStore(scene: DisneySceneState) {
...
/** True when every tile/image currently in the DOM has finished loading. */
...
function EndCardMark({ size }: { size: number }) {
  return (
    ...
    >
      <defs>
        <clipPath id="disney-endcard-clip">
      ...
      <circle
        ...
        fill="#fff"
      />
      <g clipPath="url(#disney-endcard-clip)">
        {MARK_PATHS.map((d, i) => (
...
function BrandLayer({ brand }: { brand: DisneySceneState["brand"] }) {
  return (
    <>
...
export function DisneySceneApp() {
  ...
  const [scene, setScene] = useState<DisneySceneState>(() =>
    disneySceneStateAt(0),
  );
  ...
  const applyFrame = useCallback((tMs: number) => {
    ...
    const s = disneySceneStateAt(tMs);
    const built = buildStore(s);
    useCanvasStore.setState({
      ...built,
      threads: DISNEY_THREADS,
      threadOrder: Object.keys(DISNEY_THREADS),
      activeThreadId: THREAD_CHART,
      canvasAssets: DISNEY_CANVAS_ASSETS,
  ...
  const startPlayback = useCallback(() => {
    ...
    const tick = (now: number) => {
      if (!playingRef.current) return;
      applyFrame((now - startWall) % DISNEY_DURATION_MS);
    ...
    >
      ...
      <ArtifactStyleScope styleId="liquid-glass">
        <CanvasViewport>
          ...
          />
          {/* Era text labels (real product component, t-driven reveal). */}
          {DISNEY_LABELS.map((l) => {
            ...
            .map((c) => {
              ...
              return (
                <DemoCard
                  ...
                  card={card}
                  accent={DISNEY_THREADS[c.threadId]?.accentColour ?? "#6B4EFF"}
                  viewportScale={scene.camera.scale}
                />
              );
            })}
      ...
      <CanvasBottomToolbar />
      <CanvasMinimap containerRef={containerRef} canvasKey="demo-video-disney" />
